...
class AC3:
    ...
    def consistency(self, grid, Q):
        ...
        while len(Q) != 0:
            ...
            rv, rf = self.remove_domain_row(grid, var[0], var[1])
            cv, cf = self.remove_domain_column(grid, var[0], var[1])
            uv, uf = self.remove_domain_unit(grid, var[0], var[1])
            if(rf or cf or uf):
                return False
            if rv:
                for i in rv:
...
class Backtracking:
    ...
    def search(self, grid, var_selector):
        ...
        for d in random.sample(grid.get_cells()[var[0]][var[1]], len(grid.get_cells()[var[0]][var[1]])):
            #print(d)
            copy_g = grid.copy()
            copy_g.get_cells()[var[0]][var[1]] = d
            q = [var]
            if AC3().consistency(copy_g, q):
                #print(var)
    ...
    # def consistency(self, grid, var, domain):
    #     """
    #     Check the selected variable is consistent with the gird.
    #     """   
        
    #     row = grid.get_cells()[var[0]].copy()
    #     row.pop(var[1])
    #     col = []
    #     for i in range(grid.get_width()):
    #         if (i != var[0]):
    #             col.append(grid.get_cells()[i][var[1]])
    #     block = []
    ...
    #                         for l in j:
    #                             if (var != (k, l)):
    #                                 block.append(grid.get_cells()[k][l])
    #                     break            
    #     consistent = True
    #     for i in row:
    #         if (i == domain):
...
#     g.read_file(p)
#     arc = AC3()
#     arc.pre_process_consistency(g)
#     g.print_domains()
#     print("Before:")
#     g.print()
#     b = Backtracking()
#     mrv = MRV()
#     new_g = b.search(g, mrv)
#     print("MRV:")
#     new_g.print()
i = 1
running_time_mrv = []
...
for p in problems:
    ...
    mrv_g = Backtracking().search(g, mrv)
    running_time_mrv.append(time.time() - start_time)
    start_time = time.time()
    fav_g = Backtracking().search(g, fav)
